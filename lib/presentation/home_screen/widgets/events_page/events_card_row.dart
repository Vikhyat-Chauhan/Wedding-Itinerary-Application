...
import 'package:weddingitinerary/presentation/home_screen/widgets/events_page/events_card.dart';
                ...
                children: [
                  for (int i = 0; i < state.events.length; i++)
                    if ((DateTime.now().millisecondsSinceEpoch) <
                        int.parse(state.events[i].timestamp))
                      Events_Card(
                        ...
                        scroll:
                            (state.events[i].name.length > 15) ? true : false,
                      ),
                ],
              ),
            );
